...
start_kb.add(InlineKeyboardButton('Перенос одного стиля (NST)', callback_data='1_st'))
start_kb.add(InlineKeyboardButton('Стиль Моне (GAN)', callback_data='photo2monet'))
...
settings2_st_kb.add(InlineKeyboardButton('Кол-во итераций', callback_data='num_steps'))
settings2_st_kb.add(InlineKeyboardButton('Размер изображения', callback_data='imsize'))
settings2_st_kb.add(InlineKeyboardButton('Коэффициент стиля', callback_data='style_coef'))
...
settings2_gan_kb.add(InlineKeyboardButton('Размер картинки', callback_data='imsize'))
settings2_gan_kb.add(InlineKeyboardButton('Далее', callback_data='next'))
settings2_gan_kb.add(InlineKeyboardButton('Назад', callback_data='photo2monet'))
...
async def send_welcome(message):
    await bot.send_message(
        ...
        '\n' + 'Я — Style Transfer Bot. '
               'Умею переносить стиль с картинки на картинку. ' +
        'Мои возможности:', reply_markup=start_kb)
...
async def st_1_style(callback_query):
    await bot.answer_callback_query(callback_query.id)
    await callback_query.message.edit_text("Выберите настройки для переноса стиля:")
    ...
    if photo_buffer[callback_query.from_user.id].st_type == 'photo2monet':
        await callback_query.message.edit_text(
            fmt.text(
                ...
                fmt.text(
                    "\nРазмер изображения: " +
                    str(photo_buffer[callback_query.from_user.id].settings[
                            'imsize']) + 'x' +
                    str(photo_buffer[callback_query.from_user.id].settings['imsize'])
                ),
                fmt.text(
        ...
        await callback_query.message.edit_text(
            fmt.text(
                ...
                fmt.text(
                    "\nКоличество итераций: " +
                    str(photo_buffer[callback_query.from_user.id].settings['num_steps'])
                ),
                fmt.text(
                    "\nРазмер изображения: " +
                    str(photo_buffer[callback_query.from_user.id].settings['imsize']) + 'x' +
                    str(photo_buffer[callback_query.from_user.id].settings['imsize'])
                ),
                fmt.text(
                    "\nКоэффициент стиля: " +
                    str(photo_buffer[callback_query.from_user.id].settings['style_coef']) +
                    '\n' + fmt.hbold("\nВыберите настройки для изменения:")
                )
            ), parse_mode="HTML"
        )
...
async def change_num_steps(callback_query):
    await bot.answer_callback_query(callback_query.id)
    photo_buffer[callback_query.from_user.id].settings['num_steps'] = \
        int(callback_query.data[10:])
...
async def change_style_coef(callback_query):
    await bot.answer_callback_query(callback_query.id)
    photo_buffer[callback_query.from_user.id].settings['style_coef'] = \
        float(callback_query.data[11:])
...
async def change_imsize(callback_query):
    await bot.answer_callback_query(callback_query.id)
    photo_buffer[callback_query.from_user.id].settings['imsize'] = \
        int(callback_query.data[7:])
...
async def get_image(message):
    ...
    if photo_buffer[message.chat.id].st_type == 1:
        if photo_buffer[message.chat.id].need_photos == 2:
            ...
            photo_buffer[message.chat.id].st_type == 'photo2monet' and
            photo_buffer[message.chat.id].need_photos == 1
    ):
